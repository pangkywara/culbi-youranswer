...
export default function HeroHeader({ photos, placeName, placeId }: HeroHeaderProps) {
  ...
  React.useEffect(() => {
    if (gallery && gallery.length > 0 && gallery[0] !== PLACEHOLDER) {
      Image.prefetch(gallery);
    }
  }, [gallery]);
      ...
      <SafeAreaView style={styles.overlay}>
        ...
        <View style={styles.rightBtns}>
          <TouchableOpacity style={styles.btn} onPress={onShare}>
            <ShareNetwork size={20} color="#222" />
          </TouchableOpacity>

          <TouchableOpacity 
            style={styles.btn} 
            onPress={handleHeartPress} 
            activeOpacity={1} // Prevents default button dimming from clashing with heart animation
          >
